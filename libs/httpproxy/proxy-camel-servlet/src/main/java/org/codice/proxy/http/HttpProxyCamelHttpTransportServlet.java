...
public class HttpProxyCamelHttpTransportServlet extends CamelServlet implements Externalizable {
    ...
    if (bool != null) {
      ...
      String msg =
          "Invalid parameter value for init-parameter ignoreDuplicateServletName with value: "
              + LogSanitizer.cleanAndEncode(ignore);
      LOGGER.debug(msg);
  ...
  protected void doService(HttpServletRequest oldRequest, HttpServletResponse response)
    ...
    log.trace("Service: {}", LogSanitizer.cleanAndEncode(request.toString()));
    ...
    if (consumer == null) {
      ...
      log.trace("Service Request Path = {}", LogSanitizer.cleanAndEncode(path));
      String endpointName = getEndpointNameFromPath(path);
      log.trace("Endpoint Name = {}", LogSanitizer.cleanAndEncode(endpointName));
    ...
    try {
      if (consumer == null) {
        log.debug(
            "No consumer to service request {}", LogSanitizer.cleanAndEncode(request.toString()));
      ...
      if (consumer.getEndpoint().isSuspended()) {
        log.debug(
            ...
            LogSanitizer.cleanAndEncode(request.toString()));
  ...
  protected HttpConsumer resolve(HttpServletRequest request) {
    String path = request.getPathInfo();
    log.trace("Request path is: {}", LogSanitizer.cleanAndEncode(path));
    String endpointName = getEndpointNameFromPath(path);
    log.trace("Looking up consumer for endpoint: {}", LogSanitizer.cleanAndEncode(endpointName));
