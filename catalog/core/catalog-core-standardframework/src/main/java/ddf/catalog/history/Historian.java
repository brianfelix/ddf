  ...
  private ReadStorageResponse getStorageItem(ReadStorageRequest r) {
    try {
      ...
      LOGGER.debug(
          ...
          LogSanitizer.cleanAndEncode(r.getId()),
          LogSanitizer.cleanAndEncode(r.getResourceUri().toString()),
          e);
    }
    return null;
  }
  ...
  private static Metacard firstInWinsMerge(Metacard oldMetacard, Metacard newMetacard) {
    if (LOGGER.isDebugEnabled()) {
      LOGGER.debug(
          ...
          LogSanitizer.cleanAndEncode(oldMetacard.getId()));
    }
    ...
    if (LOGGER.isTraceEnabled()) {
      ...
      LOGGER.trace(
          "Old Metacard: {}\nNew Metacard: {}",
          LogSanitizer.cleanAndEncode(metacardToString.apply(oldMetacard)),
          LogSanitizer.cleanAndEncode(metacardToString.apply(newMetacard)));
    }
