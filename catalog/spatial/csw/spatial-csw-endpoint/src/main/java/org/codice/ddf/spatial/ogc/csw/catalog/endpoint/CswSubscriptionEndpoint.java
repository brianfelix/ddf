...
public class CswSubscriptionEndpoint implements CswSubscribe, Subscriber {
  ...
  public Response createRecordsSubscription(@QueryParam("") GetRecordsRequest request)
    ...
    if (request == null) {
      ...
      LOGGER.debug(
          "{} attempting to subscribe.", LogSanitizer.cleanAndEncode(request.getRequest()));
    }
  ...
  public synchronized boolean hasSubscription(String subscriptionId) {
    LOGGER.debug("subscriptionUuid = {}", LogSanitizer.cleanAndEncode(subscriptionId));
  ...
  private synchronized CswSubscription deleteCswSubscription(String subscriptionId)
    ...
    LogSanitizer logSanitizedId = LogSanitizer.cleanAndEncode(subscriptionId);
    LOGGER.trace("ENTERING: {}", methodName);
    LOGGER.trace("subscriptionId = {}", LogSanitizer.cleanAndEncode(subscriptionId));
  ...
  private Configuration getSubscriptionConfiguration(String subscriptionUuid) {
    ...
    LogSanitizer logSanitizedFilter = LogSanitizer.cleanAndEncode(filterStr);
